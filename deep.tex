...
% University of Malakand Thesis – Final Polished Modern Version
...
\usepackage{caption}
\usepackage{multicol}       % only for abbreviation list
\usepackage{qrcode}         % for modern title page
...
\definecolor{Primary}{HTML}{1B2A3A}      % dark navy
\definecolor{Accent}{HTML}{3C78D8}       % bright blue
\definecolor{Light}{HTML}{F5F7FA}        % very light grey
...
% ===== CLEAN CHAPTER STYLE (dark rule below) =====
\titleformat{\chapter}[display]
  {\normalfont\huge\bfseries\color{Primary}}
  {\chaptertitlename\ \thechapter}
  {20pt}
  {\Huge}
  [{\vspace{-10pt}\color{Primary}\rule{\textwidth}{3pt}}]
...
\titleformat{\section}
  ...
  {\thesection}{1em}{}
  [{\color{Primary!40}\rule{\textwidth}{0.8pt}}]
\titlespacing*{\section}{0pt}{14pt}{6pt}

\titleformat{\subsection}
  {\normalfont\large\bfseries\color{Primary!90}}
  {\thesubsection}{1em}{}
\titlespacing*{\subsection}{0pt}{10pt}{4pt}

% ===== PAGE HEADER (light, accent line) =====
\pagestyle{fancy}
\fancyhf{}
\renewcommand{\headrulewidth}{0pt}
\fancyhead[L]{\footnotesize\color{Primary!70}\thesistitle}
\fancyhead[R]{\footnotesize\color{Primary!70}\thesisuni}
\fancyfoot[C]{\thepage}
% add a thin accent rule below the header
\fancyhead[C]{}
...
\newcommand{\thesisauthor}{Sajjad Alam,Sajid khan,Ihsan Ullah}
...
% ===== MODERN TITLE PAGE – perfectly aligned =====
% ===== MODERN YET FORMAL THESIS TITLE PAGE =====
\begin{titlepage}
\thispagestyle{empty}

% Dark banner – taller, title fits nicely
\begin{tikzpicture}[remember picture, overlay]
  \fill[Primary] (current page.north west) rectangle ([yshift=-5.0cm]current page.north east);
  \node[anchor=north, yshift=-2.2cm, text width=0.95\paperwidth, align=center]
    at (current page.north)
    {\color{white}\fontsize{20}{34}\selectfont\bfseries\thesistitle};
\end{tikzpicture}

\vspace{4.5cm} % push content below banner

% Formal centered block
\begin{center}
  {\LARGE\bfseries\color{Primary} By}\\[18pt]
  {\LARGE\bfseries\color{Primary} Sajjad Alam}\\[6pt]
  {\LARGE\bfseries\color{Primary} Ihsan Ullah}\\[6pt]
  {\LARGE\bfseries\color{Primary} Sajid Khan}\\[30pt]

  {\large A thesis submitted in partial fulfillment of the requirements for the degree of}\\[12pt]
  {\Large\bfseries\color{Primary} \thesisdegree}\\[30pt]

  {\large\color{Primary!80} \thesisdept}\\[4pt]
  {\large\color{Primary!80} \thesisuni}\\[20pt]
  {\Large\bfseries\color{Primary} \thesisyear}
\end{center}

\vfill

% Bottom left: subtle live metrics + QR (pushed to bottom right)
\begin{flushright}
  \begin{tcolorbox}[enhanced,boxrule=0.3pt,colback=Accent!8,colframe=Accent,
    width=0.40\textwidth,arc=3pt,fontupper=\small\bfseries,
    title=Live Metrics,center title,fonttitle=\small]
    Best AUC: \textbf{0.8023} \hspace{0.3cm} API: \textbf{Active}
  \end{tcolorbox}
  \vspace{4pt}
  \qrcode[height=2.2cm]{https://github.com/AlamSajjad456/R1}
\end{flushright}
\end{titlepage}
...
% Uniform style for frontmatter certificate pages
\newenvironment{certpage}[1]{%
  \clearpage
  \thispagestyle{empty}
  \vspace*{1.5cm}
  \begin{tcolorbox}[enhanced,boxrule=0pt,colback=Primary!5,width=\textwidth,
    arc=4pt,before skip=0pt,after skip=0pt,top=10pt,bottom=10pt,
    title={\centering\bfseries\large\color{Primary!80}#1},center title,
    colbacktitle=Primary!10,fonttitle=\bfseries\large]
}{\end{tcolorbox}\clearpage}

% ----- DECLARATION -----
\begin{certpage}{Declaration}
I, Sajjad Alam, Sajid Khan, and Ihsan Ullah, hereby declare that this thesis titled ``\thesistitle'' is our own work and has not been submitted previously for any other degree or professional qualification.

\par\bigskip
Signature: \dotfill \hspace{2cm} Date: \dotfill
\end{certpage}

% ----- PLAGIARISM UNDERTAKING -----
\begin{certpage}{Plagiarism Undertaking}
...
\begin{certpage}{Submission Authorization Certificate (For Evaluation)}
Certified that the thesis of Sajjad Alam, Sajid Khan, and Ihsan Ullah entitled ``\thesistitle'' has been reviewed in final form and permission is granted to submit it to \thesisuni\ for evaluation.
...
\begin{certpage}{Approval Certificate (For the Award of Degree)}
It is certified that the work contained in this thesis entitled ``\thesistitle'' was carried out by Sajjad Alam, Sajid Khan, and Ihsan Ullah under supervision and is adequate in scope and quality for the award of the degree.
...
\begin{center}
\emph{To my parents who always believed in me.}
\end{center}
...
Cardiovascular disease CVD for short is still one of the top causes of death around the world. Every year millions of people die from heart failure coronary artery disease or stroke. That's what the WHO keeps telling us. These problems are usually tied to lifestyle: eating badly not exercising smoking diabetes high blood pressure and being overweight. So yeah catching it early really matters. The earlier you find it the better the treatment can work.

Normally doctors diagnose based on their own experience lab tests and clinical guidelines. That works fine but sometimes it misses the subtle patterns hiding inside medical data. Nowadays we have more healthcare data than ever and computers are fast so machine learning has become a good way to dig through large medical datasets and help predict diseases.

ML algorithms learn from past examples and then make predictions on new unseen data. In healthcare that means they can help doctors spot high‑risk patients and support early diagnosis. People have already used models like Logistic Regression Random Forest Support Vector Machines and Gradient Boosting for all kinds of medical prediction tasks.

The dataset I used here includes age gender blood pressure cholesterol glucose smoking drinking and physical activity. By looking at those features the ML models try to find patterns that might signal whether someone is likely to get CVD.

Using machine learning in healthcare could make diagnoses more accurate cut costs and give doctors a second opinion. That's why building a reliable CVD prediction model is worth working on and that's exactly what I did in this thesis.

\section{Problem Statement}
Medical science has improved a lot no doubt. But heart disease still kills tons of people around the world. Catching it early and stopping it before it gets bad that's really important. The thing is traditional diagnosis mostly relies on doctors looking at clinical data by hand. That takes a while and honestly people mess up sometimes.

Medical datasets usually have a bunch of variables and all these complicated relationships between them. Old school stats models just can't pick up the nonlinear patterns hidden in the risk factors. So yeah we really need smart systems that can dig through large amounts of medical data and give us accurate predictions about heart disease risk.

Machine learning can learn patterns from data all by itself. That helps get around the limits of traditional methods. But picking the right algorithm and actually checking how well it works that's still tricky. So that's why I'm doing this study. I want to build and test different ML models to predict cardiovascular disease using real clinical data.
...
This study shows that ML can help predict heart disease. If it works well doctors could use it to catch high‑risk patients early and maybe prevent bad outcomes. The model could be a decision support tool not replacing doctors just helping them. Also I hope this research encourages more people to try AI in healthcare.

\section{Scope of the Study}
I focused only on one structured dataset. I did all the usual steps: preprocessing exploring the data training models and evaluating them. I did not deploy the model in a real hospital. That's something for future work.

\section{Structure of the Thesis}
Chapter 2 is a literature review what other people did. Chapter 3 explains my methods. Chapter 4 shows the results. Chapter 5 wraps everything up and talks about what could be done next.

\begin{takeaway}
\textbf{Key points from Chapter 1:}
\begin{itemize}
  \item CVD remains a leading cause of death; early detection is critical.
  \item ML can uncover hidden patterns in clinical data, aiding diagnosis.
  \item This thesis develops and compares seven ML models for CVD prediction.
  \item The goal is a deployable, explainable screening tool.
\end{itemize}
\end{takeaway}
...
Let me break down each algorithm's strengths and weaknesses in my own words:
\begin{itemize}
\item \textbf{Logistic Regression:} Simple fast interpretable. But it assumes a linear relationship.
\item \textbf{Decision Trees:} Easy to visualise, prone to overfitting.
\item \textbf{Random Forest:} Ensemble of trees, robust, reduces overfitting.
\item \textbf{SVM:} Good in high dimensions, but tuning can be tricky.
\item \textbf{Gradient Boosting (XGBoost, LightGBM):} Often best for tabular data.
\item \textbf{Neural Networks:} Flexible, need careful tuning, less interpretable.
\end{itemize}
...
% ================== CHAPTER 3: RESEARCH METHODOLOGY (expanded, humanized, ~15 pages) ==================
...
I didn't have a fancy GPU server or a cloud cluster. Everything ran on my Dell Inspiron laptop with an 11th‑gen Intel i5 processor and 16 GB of RAM, running Windows 11. The Python version was 3.10, and I used scikit‑learn 1.2 for the traditional models, XGBoost 1.7.4, LightGBM 3.3.5, and TensorFlow 2.11 for the neural net. For EDA I relied on pandas, numpy, matplotlib, and seaborn.
...
\end{takeaway}
% ================== CHAPTER 4 (single column, unified colors) ==================
...
% ================== CHAPTER 5: CONCLUSION AND FUTURE WORK (expanded paragraph style) ==================
...
\bibitem{who_cvd} World Health Organization. Cardiovascular diseases (CVDs). WHO Fact Sheets 2026.
\bibitem{kaggle_cardio} Kaggle. Cardiovascular Disease Dataset (sulianova/cardiovascular-disease-dataset). Accessed 2026.
\bibitem{xgboost} T. Chen and C. Guestrin. XGBoost: A Scalable Tree Boosting System. Proceedings of the 22nd ACM SIGKDD 2016.
...
\bibitem{breiman2001random} L. Breiman. Random Forests. Machine Learning 45(1):5–32 2001.
\bibitem{detrano1989international} R. Detrano et al. International application of a new probability algorithm for the diagnosis of coronary artery disease. American Journal of Cardiology 64(5):304–310 1989.
\end{thebibliography}
...
- `api.py` – runs the FastAPI server.
